<?php
    ...
    if(isset($_POST["change-quantity"])){
        ...
        {
            //echo "<script>alert('Session variable is set')</script>";
            //echo "<script>window.location = index.php</script>";
            $item_quantity_id = array_column($_SESSION["product_quantity"], "product_id");
            print_r($item_quantity_id);
            echo "<script>alert('Session variable is set')</script>";
            if(!in_array($_POST["product_id"],$item_quantity_id))
            {
                print_r(" Not added");
                echo "<script>alert('Product is not added')</script>";
                //unset($_SESSION["product_quantity"][$quantity_count]);
                $quantity_count = count($_SESSION["product_quantity"]);
                $item_id = $_POST["product_id"];
                print_r("\n");
                print_r($quantity_count);
                $item_array_quantity = array(
                    'product_id'    => $_POST["product_id"],
                    'product_quantity' => $_POST["quantity"],
                    'product_price'  => $_POST["product_price"],
                    'final_price' => $_POST["product_price"] * $_POST["quantity"]
                );
                //print_r($item_array_quantity);
                $_SESSION["product_quantity"][$item_id] = $item_array_quantity;
                ...
                $item_id = $_POST["product_id"];
                unset($_SESSION["product_quantity"][$item_id]);
                $item_array_quantity = array(
                    'product_id'    => $_POST["product_id"],
                    'product_quantity' => $_POST["quantity"],
                    'product_price'  => $_POST["product_price"],
                ...
                print_r("\n");
                //print_r($item_array_quantity['final_price']);
                $_SESSION["product_quantity"][$item_id] = $item_array_quantity;
                print_r($_SESSION["product_quantity"]);
            }
        }else{
                                            ...
                                            <div class="borders">
                                                <div class="row cart-container">
                                                    <div class="col-md-3 cart-img">
                                                        <img src="../mobile/shoppingdemo/image/'.$row["product_image"].'" alt="">
                                                    </div>
                                                    <div class="col-md-6">
                                                        <h5 class="pt-2">'.$row["product_name"].'</h5>
                                                        <small class="text-secondary">Seller:Akash Singh</small>
                                                        <h5 class="pt-2">'.$row["product_price"].'</h5>
                                                        <h5 class="pt-2"> Total Price: '.$final_price = ($_SESSION["product_quantity"][$row["product_id"]]["final_price"]).'</h5>
